...
class landed_cost_position(orm.Model):
    ...
    _columns = {
        ...
        'company_id': fields.related(
            ...
            store=True,
            readonly=True),
#        'landingcost_currency_id': fields.many2one(
#            'res.currency',
#            'Currency',
#            help="Landing Cost Currency if different from Purchase's Order",
#            required=True),
    }
...
class purchase_order(orm.Model):
    ...
    def _prepare_landed_cost_inv_line(self, cr, uid, account_id, inv_id,
        ...
        **** cambie 'price_unit': landed_cost.amount por landed_cost.amount_company_currency
        """
        ...
        return {
            ...
            'account_id': account_id,
            'invoice_id': inv_id,
            'price_unit': landed_cost.amount_company_currency or 0.0,
            'quantity': qty,
            'product_id': landed_cost.product_id.id or False,
            'invoice_line_tax_id': [(6, 0, line_tax_ids)],
        }

    def _prepare_landed_cost_inv(self, cr
                                 , uid, landed_cost, context=None):
        """ Collects require data from landed cost position that is used to
        ...
        **** cambie currency_id = landed_cost.purchase_order_id.pricelist_id.currency_id.id
                por landed_cost.purchase_order_id.company_id.currency_id.id
        """
        po = (landed_cost.purchase_order_id or
              landed_cost.purchase_order_line_id.order_id)
        currency_id = landed_cost.purchase_order_id.company_id.currency_id.id
    ...
    def _generate_invoice_from_landed_cost(self, cr, uid, landed_cost,
        ...
        """ Para afectar de forma correcta la cuenta contable de inventario,
            necesito afectar la misma que cuenta que la del artículo. El original
            toma la cuenta contable del Landed Cost, aquí lo obligo a que lo haga del
            articulo en la orden
        ...
        """
        print "*** EL PRODUCTO A MANDAR ES: ", po.order_line[0].product_id.id
        exp_account_id = prod_obj._choose_valuation_account_from(
            cr, uid,
            po.order_line[0].product_id,
            fiscal_position=fiscal_position,
            context=context
        )
