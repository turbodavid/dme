...
class product_product(orm.Model):
    ...
    def _choose_valuation_account_from(self, cr, uid, product, fiscal_position=False,
                                 context=None):
        """ Method to compute the inventory valuation account to chose based on product and
        fiscal position.
        ...
        property_obj = self.pool.get('ir.property')
        if product:
            acc_id = product.categ_id.property_stock_valuation_account_id.id
            if not acc_id:
                raise orm.except_orm(
                    _('Error!'),
                    _('Define stock valuation account for this company: "%s" (id:%d).')
                    % (product.name, product.id,))
        else:
            acc_id = property_obj.get(cr, uid,
