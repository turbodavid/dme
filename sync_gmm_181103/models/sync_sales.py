...
class sync_morsa_sales(models.Model):
    ...
    cuenta_ventas_contado = fields.Many2one(
                'account.account',
                    default=2157,
                    domain=[('code', 'like','410-1'), ('type','!=', 'view')],
                    required=True
    )
        ...
        if bd[0:3] == 'GMM':
            ...
            for unidad in unidades:
                try:
                    ...
                    con = psycopg2.connect("host='%s' port='5432' dbname='%s' user='kerberox' password='alkatraz'"%(unit.ip_address,unit.data_base))
                    cursor = con.cursor()
